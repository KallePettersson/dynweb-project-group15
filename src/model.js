...
class GeoLocoModel {
  ...
  removeObserver(callback) {
    this.observers = this.observers.filter(
      (obs) => obs.toString() !== callback.toString()
    );
  }
  ...
  metaDataSetup() {
    return {
      ...
      keyToString: {
        ...
        health_care_index: "Health Care Index",
        pollution_index: "Pollution Index",
        property_price_to_income_ratio: "Property Price Ratio",
        purchasing_power_incl_rent_index: "Purchasing Power Index",
        rent_index: "Rent Index",
        restaurant_price_index: "Restaurant Price Index",
        safety_index: "Safety Index",
        traffic_co2_index: "Traffic CO2 Index",
        traffic_index: "Traffic Index",
        traffic_inefficiency_index: "Traffic Inefficiency Index",
        traffic_time_index: "Traffic Time Index",
      },
      keyToInfo: {
  ...
  setupForEachCountryDataPoint() {
    if (this.countryData !== null) {
      console.log(
        "this.countryData - setupForEachCountryDataPoint",
        this.countryData
      );
      console.log(
        "this.countryData - setupForEachCountryDataPoint",
      ...
      console.log(
        "this.countryData - setupForEachCountryDataPoint",
        Object.entries(this.countryData).length
      );
      let newCountryData = {};
      Object.entries(this.countryData).forEach((country) => {
        newCountryData[country[0]] = country[1];
        newCountryData[country[0]]["fillKey"] = this.getColourGradient(
          country[1][this.metaData.currentDataKey]
        ); // Set colour for given country

        // Extra data needed for popup
        newCountryData[country[0]]["currentDataKey"] =
          this.metaData.currentDataKey;
  ...
  setupForEachCityDataPoint() {
    ...
    Object.entries(this.cityData["cities"]).forEach((city) => {
      ...
      city[1].fillKey = this.getColourGradient(
        city[1][this.metaData.currentDataKey]
      ); // Set colour for given country
      city[1].radius = this.getCityRadius(
        city[1][this.metaData.currentDataKey]
      ); // Set radius of city bubble
      // Extra data needed for popup
