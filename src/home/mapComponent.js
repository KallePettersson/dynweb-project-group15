...
function MapComponent(props) {
  ...
  var map = new Datamap({
    ...
    setProjection:
      props.metaData.countrySelected !== "World" ? zoomToCountry : null, //Function set if country-view is selected
  });
  ...
  if (props.metaData.countrySelected !== "World") {
    ...
    //Re-color map
    resetAllColorsExcept(
      map,
      props.countryData,
      props.metaData.countrySelected
    );
  }
...
function countryPopupTemplate(geo, data) {
  return [
    ...
    data
      ? "<br/>" +
        data.keyToString[data.currentDataKey] +
        " : " +
        data[data.currentDataKey].toFixed(2)
      : "<br/> No data",
    "</div>",
  ].join("");
}
